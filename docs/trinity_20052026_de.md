...
- **`grade_assistant_agent`**: Generiert kriterienbasierte Entwürfe für Gutachten im Office Mode. Der Agent stützt sich auf hochschulspezifische Bewertungsmatrizen und gleicht studentische Abgaben mit den Modulleistungen ab.
...
- **`style_checker_agent`**: Analysiert Writing Samples des Professors und passt die automatisch generierten Gutachten-Formulierungen stilistisch an dessen persönlichen Schreibstil an.
...
- **`grade_exporter_agent`**: Überführt erstellte Gutachten-Noten strukturiert in Excel- oder CSV-Dateien für das Prüfungsamt.
...
- **Office Mode (Büro):** In diesem Modus unterstützt TRINITY den Dozenten bei administrativen Aufgaben. Über eine intuitive Drag-&-Drop-Schnittstelle in der PySide6-Desktop-App können Seminararbeiten oder studentische Meilensteine eingepflegt werden. TRINITY analysiert diese Dokumente layout-sensitiv und generiert proaktiv strukturierte Gutachten-Entwürfe (Anonymous 2026). Hierbei wird die Einhaltung didaktischer Kriterien durch eine gezielte Prompt-Verankerung sichergestellt, um faire und reproduzierbare Bewertungen zu erzeugen.
