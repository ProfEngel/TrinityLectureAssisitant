...
Even G2 is intentionally an STT-only input device. Completed 16 kHz PCM
utterances use the bridge endpoint `/audio/transcribe`; they do not reserve a
full Eve realtime conversation slot. On Apple Silicon the bridge automatically
uses MLX Parakeet. The Windows remote profile delegates G2 transcription to a
dedicated authenticated Parakeet endpoint on the Ubuntu host (`8767`) and
retains portable CPU Whisper only as a failover. The Eve realtime endpoint
(`8766`) is therefore never occupied by a G2-only transcription. Eve TTS
is generated by Trinity Desktop and played only by the
centrally selected iPhone, iPad, Mac, or Windows output device.
