...
def test_macos_installer_preserves_canvas_reference_without_installing_it():
    script = INSTALLER.read_text(encoding="utf-8")
    ...
    assert 'npm ci && npm run build' not in script
    assert "Creative Canvas pausiert" in script
